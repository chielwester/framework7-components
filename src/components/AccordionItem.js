(() => ({
  ...
  jsx: (() => {
    ...
    const F7AccordionItem = (
      <ListItem accordionItem title={env === 'dev' ? options.title.join(' ') : useText(options.title)}>
        <AccordionContent>
          <Block>
            {children}
            <p>
    ...
    return env === 'dev' ? <div className={classes.root}>{F7AccordionItem}</div> : F7AccordionItem;
  })(),
  styles: () => () => ({
    root: {},
  }),
